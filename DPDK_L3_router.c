...
static void handle_ipv4(struct rte_mbuf *mbuf, uint16_t in_port, struct rte_mempool *pool)
{
    ...
    /* Resolve next-hop MAC (the destination host itself, since it's directly
     * attached to that port's subnet) */
    struct rte_ether_addr next_hop_mac;
    if (!arp_lookup(out_port, dst_ip, &next_hop_mac)) {
        char dstr[16];
        ip_to_str(dst_ip, dstr, sizeof(dstr));
        printf("[ARP MISS] Don't know MAC for %s yet -- requesting, dropping this packet\n", dstr);
